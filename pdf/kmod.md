...
#### insmod()
这里所实现的 insmod 函数，和之前的 insmod 命令不同，这个函数能够实现对当前模块所依赖的所有模块的 probe 方式加载。
其实也就是实现 modprobe 命令的核心函数。其中所用到的实现加载功能的核心函数是 kmod_module_probe_insert_module 。

对比一下 insmod 命令当时的实现代码，其用到的核心函数是 kmod_module_insert_module ，这个函数只能加载一个模块，
不能实现对所有依赖关系模块的加载，因此功能上来说，kmod_module_probe_insert_module 需要调用 kmod_module_insert_module 来完成真正的加载。
总的来说，前者的完成需要实现一个依赖关系的列表，然后通过循环调用多次后者，才能完成 probe 方式加载。
...
![插入130行的打印 line 的语句](./pictures/3-1-depline.png)
...
![make编译成功，增量编译了libkmod-module模块](./pictures/3-2-make.png)
...
![运行 modprobe nfs 完成插入](./pictures/3-3-modprobe.png)
...
![插入打印每个模块名称的调试代码](./pictures/3-4-print-p.png)
...
![调试代码输出详细调试信息](./pictures/3-5-print-p-output.png)
