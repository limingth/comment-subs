...
THU-12-1
测试用例说明
本用例用于测试在 uBuntu 12.04 上，如何下载安装 kmod-11 项目。
初始化要求
硬件环境 型号：Macbook Air 笔记本电脑 CPU：Intel Core 2 Duo
1.4GHz 内存：2GB DDR3 硬盘：64GB
软件环境 已安装 Vmware Fusion 虚拟机 5.0.3, 虚拟机安装有Ubuntu 12.04.2
LTS，其中安装有GCC版本 4.6.3
前提和约束
无
测试过程
序号
...
1
详细步骤
修改gcc编译选项，使其不生成.note.gnu.build-id段
输入数据
CC=``gcc -Wl,--build-id=none'' ./configure --prefix=/opt/grub/
预期结果
在GRUB源代码目录下生成Makefile文件
2
详细步骤
输入make命令，构建二进制文件
输入数据
make
预期结果
生成stage1，stage1\_5（e2fs\_stage1\_5， fat\_stage1\_5， ffs\_stage1\_5，
iso9660\_stage1\_5， jfs\_stage1\_5， minix\_stage1\_5，
reiserfs\_stage1\_5 ，ufs2\_stage1\_5， vstafs\_stage1\_5，
xfs\_stage1\_5），以及stage2这些二进制映像。
3
详细步骤
输入make install命令，将二进制文件安装到指定目录
输入数据
make install
预期结果
在/opt/grub/下生成以下目录：bin， info， lib， man，sbin
过程终止条件
make install成功并在/opt/grub/下生成以下目录：bin， info， lib， man，sbin
结果判定标准
以上各步骤都成功，并没有错误产生。
