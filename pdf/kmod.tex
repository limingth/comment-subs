...
\paragraph{configure 脚本执行时缺少 libtool 工具}
...
\paragraph{configuire 脚本执行缺少 xsltproc 命令}
...
\paragraph{struct kmod\_ctx 结构体定义}
...
\paragraph{struct kmod\_module 结构体定义}
...
\paragraph{struct kmod\_file 结构体定义}
...
\paragraph{struct kmod\_elf 结构体定义}
...
\paragraph{struct kmod\_list 结构体定义}
...
\paragraph{struct kmod\_config 结构体定义}
...
\paragraph{struct index\_mm 结构体定义}
...
\paragraph{struct kmod\_elf\_header 结构体定义}
...
\paragraph{struct list\_node 结构体定义}
...
\end{verbatim}\end{shaded}}
\subsubsection{模块加载后运行 ls 命令没有输出}
...
clean:
    rm -rf *.o *.ko *.mod.* *.cmd 
\end{verbatim}\end{shaded}}
\subsubsection{卸载模块后运行 ls 输出恢复正常}
...
\end{verbatim}\end{shaded}}
\subsubsection{运行 kill 试图杀死当前 bash 不能成功}
