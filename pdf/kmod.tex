...
\item
  ...
  \$ cat /etc/modprobe.d/myalias.conf alias mymod
  really\_long\_module\_name
\item
  blacklist 黑名单 例如

  \$ cat /etc/modprobe.d/blacklist blacklist ieee1394 blacklist ohci1394
  blacklist eth1394 blacklist sbp2
\item
  install 和 remove 命令 例如

  \$ cat /etc/modprobe.d/ieee1394 install ieee1394 /bin/true install
  ohci1394 /bin/true install eth1394 /bin/true install sbp2 /bin/true
\end{itemize}
\subsubsection{库接口层}
...
\item
  ...
  \end{itemize}
\end{itemize}
\subsubsection{头文件 libkmod.h 分析}
...
\end{verbatim}\end{shaded}}
\subsubsection{kmod\_module\_probe\_insert\_module 核心代码分析}
...
以下我们通过3个例子，来说明这种安全漏洞如何实现和解决。

\subsection{安全漏洞案例1 ls命令}

我们经常使用的用户空间的命令，大部分都是通过系统调用来完成的。例如 ls
显示目录内容的命令，我们可以通过 strace 来跟踪程序使用了何种系统调用。

\subsubsection{strace 跟踪 ls 命令的系统调用}
...
write(1, "aclocal.m4   config.log     COPY"..., 67aclocal.m4   config.log     COPYING  Makefile   NEWS      testsuite
) = 67
write(1, "build-aux    config.status  libk"..., 65build-aux    config.status  libkmod  Makefile.am  README    TODO
) = 65
write(1, "config.h     configure\t    libto"..., 65config.h     configure       libtool  Makefile.in  stamp-h1  tools
) = 65
write(1, "config.h.in  configure.ac   m4\t "..., 47config.h.in  configure.ac   m4        man      tags
) = 47
close(1)                                = 0
...
$ 
\end{verbatim}\end{shaded}}
\subsubsection{清空系统日志信息以便查看模块加载信息}

{\begin{shaded}\begin{verbatim}
$ sudo dmesg -C
$ dmesg 
\end{verbatim}\end{shaded}}
\subsubsection{插入模块打印加载提示信息}
...
\end{verbatim}\end{shaded}}
参考资料：
\url{http://hi.baidu.com/lu_youyou/item/b6585ff84ade0b1ea62988c9}
